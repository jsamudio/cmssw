 ...
 */
  constexpr inline bool is_work_lane(const unsigned int work_mask, const unsigned int lane_id, const unsigned extent){ 
    if (lane_id >= extent) return false;
 ...
 */
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE unsigned int warp_exclusive_sum(TAcc const& acc, unsigned int val, const unsigned int lane_idx) {
    ...
    for (unsigned int step = 1; step < w_extent; step *= 2) {
      ...
      if (lane_idx >= step) local_offset += res;
    }
 ...
 */
  ...
  ALPAKA_FN_HOST_ACC ALPAKA_FN_INLINE unsigned int get_logical_lane_idx(TAcc const& acc, const unsigned int mask, const unsigned int lane_idx) {
    if(lane_idx  == 0) return lane_idx; // nothing to do, phys idx coincide with the logical one.
...
template< typename TAcc, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>>
ALPAKA_FN_ACC ALPAKA_FN_INLINE unsigned int get_physical_lane_idx(TAcc const& acc, const unsigned int mask, int logical_lane_idx) {
  int m = mask;

  while (logical_lane_idx--) m &= (m - 1);
 ...
 */
  ...
  {   
    ...
    if constexpr (all) result = alpaka::warp::shfl(acc, result, 0, w_extent);
 ...
 */
  ...
  template< typename TAcc, typename reduce_t, typename reducer_t, bool all = true, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>>
  requires std::is_arithmetic_v<reduce_t>
  ALPAKA_FN_ACC ALPAKA_FN_INLINE reduce_t warp_sparse_reduce(TAcc const& acc, const unsigned int mask, const unsigned int lane_idx, reduce_t const in, const reducer_t f)
  { 
    unsigned int const w_extent = alpaka::warp::getSize(acc);

    // Non-active lanes should skip the reduction:
    if (is_work_lane(mask, lane_idx, w_extent) == false) return in;
    ...
    for (unsigned int offset = pow2_boundary / 2; offset > 0; offset /= 2) {
      ...
      const unsigned int src_lane_idx  = (logical_src_lane_idx < nActiveLanes) ? get_physical_lane_idx(acc, mask,logical_src_lane_idx) : lane_idx;

      const reduce_t neigh_res = warp::shfl_mask(acc, mask, res, src_lane_idx, w_extent);

      if (logical_src_lane_idx < nActiveLanes) res = f(res, neigh_res);
 ...
 */
  ...
  template< typename TAcc, bool all = true, typename = std::enable_if_t<alpaka::isAccelerator<TAcc>>>
  ALPAKA_FN_ACC ALPAKA_FN_INLINE unsigned int warp_sparse_exclusive_sum(TAcc const& acc, const unsigned int mask, const unsigned int val, const unsigned int lane_idx)
  { 
    unsigned int const w_extent = alpaka::warp::getSize(acc);

    // Non-active lanes should skip the reduction:
    if (is_work_lane(mask, lane_idx, w_extent) == false) return 0;

    const unsigned int nActiveLanes = alpaka::popcount(acc, mask); // count number of active lanes
    // First check if this is just a single active lane in the warp:
    if (nActiveLanes == 1) return val; //nothing to do, note that this is the inclusive "sum": low lane always keeps the whole sum 
    ...
    for (unsigned int step = 1; step < pow2_boundary; step *= 2) {
      const unsigned int src_lane_idx  = (logical_lane_idx >= step) ? get_physical_lane_idx(acc, mask,logical_lane_idx-step) : logical_lane_idx;
      const unsigned int tmp_val = warp::shfl_mask(acc, mask, local_offset, src_lane_idx, w_extent);

      if (logical_lane_idx >= step) local_offset += tmp_val; 
    }
    ...
    if constexpr (all) {
      ...
      const unsigned int low_lane_idx  = get_physical_lane_idx(acc, mask, 0);
    	...
    	if (logical_lane_idx == 0) local_offset = tmp; //lane 0 keeps full (inclusive for the last lane) sum

    	warp::syncWarpThreads_mask(acc, mask);
    }
    return logical_lane_idx == 0 ? local_offset : local_offset - val; //we return exclusive sum, except zero logical lane (which returns total offset)    
  }
}
