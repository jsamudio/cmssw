 ...
 */
  ...
  public:
    ...
    ALPAKA_FN_ACC void operator()(TAcc const& acc,
      ...
      auto& connected_comp_sizes(alpaka::declareSharedVar<unsigned int[max_w_items * max_w_extent + 1], __COUNTER__>(acc));

      auto& connected_comp_rhf_offsets(alpaka::declareSharedVar<int[max_w_items * max_w_extent + 1], __COUNTER__>(acc));
      auto& connected_comp_rhf_sizes(alpaka::declareSharedVar<unsigned int[max_w_items * max_w_extent + 1], __COUNTER__>(acc));
      ...
      for (auto group : ::cms::alpakatools::uniform_groups(acc)) { 
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nVertices, w_extent))) {
          ...
          if (idx.local < max_w_items) subcc_offsets[idx.local] = 0;
        }        
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nVertices, w_extent))) {          
          ...
          if ( is_work_lane(rep_mask, lane_idx, w_extent) == false ) continue;
          ...
          if (lane_idx == low_rep_idx) {
            ...
            local_topo_offset = alpaka::atomicAdd(acc, &component_map[nVertices], local_n_topo, alpaka::hierarchy::Threads{});
          } 
          warp::syncWarpThreads_mask(acc, rep_mask);
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nVertices, w_extent))) {          
          ...
          const unsigned int subcomp_rhf_size = warp_sparse_exclusive_sum(acc, subcomp_mask, rhf_size, lane_idx);

          unsigned int relative_rhf_offset_stub = 0;

          if (lane_idx == local_subcomponent_rep_idx) {                
            relative_rhf_offset_stub = alpaka::atomicAdd(acc, &connected_comp_rhf_sizes[rep_idx], subcomp_rhf_size, alpaka::hierarchy::Threads{}); 
          }
          warp::syncWarpThreads_mask(acc, active_lanes_mask);

          const unsigned int relative_rhf_offset = warp::shfl_mask( acc, subcomp_mask, relative_rhf_offset_stub,local_subcomponent_rep_idx, w_extent );

          connected_comp_rhf_offsets[vertex_idx] = relative_rhf_offset;// store relative offsets
        }        
        alpaka::syncBlockThreads(acc);
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nVertices, w_extent))) {          
          ...
          if ( is_work_lane(rep_mask, lane_idx, w_extent) ) {
            const unsigned int cc_idx = component_map[lane_idx];
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nComponents, w_extent))) {          
          const unsigned int active_lanes_mask = alpaka::warp::ballot(acc, idx.local < nComponents);
          
          if (idx.local > nComponents) continue;
          ...
          if(lane_idx == 0) subcc_offsets[warp_idx] = cc_rhf_offset;
        }
        alpaka::syncBlockThreads(acc);
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nVertices, w_extent))) { 
          ...
          if (lane_idx == low_local_rep_idx) {
            ...
            if (probe_energy == max_energy) cc_seeds[cc_idx] = vertex_seeds[vertex_idx];
          }
        }
        alpaka::syncBlockThreads(acc);
        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, ::cms::alpakatools::round_up_by(nComponents, w_extent))) { 
          ...
          outPFCluster[topo_idx].topoId() = pfCluster[root_idx].topoId();// might be wrong: merged clusters could have different topoIds? Now I just keep one from root.
          ...
          outPFCluster[topo_idx].rhfracSize() = cc_rhf_offsets[topo_idx];
