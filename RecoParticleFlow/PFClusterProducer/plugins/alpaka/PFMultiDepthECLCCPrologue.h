...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  // Perform warp level exclusive sum with pre-defined lane mask:
  ...
  class ECLCCPrologueKernel {
    public:
      ...
      ALPAKA_FN_ACC void operator()(TAcc const& acc,
        ...
        //
        auto& outer_neigh_masks(alpaka::declareSharedVar<VecArray<int, max_w_items * max_w_extent>, __COUNTER__>(acc)); 
        auto& inner_neigh_masks(alpaka::declareSharedVar<VecArray<int, max_w_items * max_w_extent>, __COUNTER__>(acc));
        //
        auto& base_neighbor(alpaka::declareSharedVar<VecArray<int, max_w_items * max_w_extent>, __COUNTER__>(acc));
        // Neighbor list offset
        auto& nlist_offsets(alpaka::declareSharedVar<VecArray<unsigned int, (max_w_items * max_w_extent + 1)>, __COUNTER__>(acc));
        //
        auto& adjacency_list(alpaka::declareSharedVar<VecArray<int,2*max_w_items * max_w_extent>, __COUNTER__>(acc));
        // Alias:
        ...
        for ( auto group : uniform_groups(acc) ) {//loop over thread blocks
          ...
          for( auto idx : uniform_group_elements(acc, group, round_up_by(nVertices, w_extent)) ) { 
            ...
            const int dst_vertex_idx = idx.local; 
            // 
            ...
            //
            const int src_vertex_idx = pfClusteringVars[idx.global].mdpf_topoId();
            ...
            const int src_vertex_lane_idx = is_local_src_vertex_idx ? src_vertex_idx % w_extent : -1;
            ...
            const auto rep_lane_idx = ( ((control_mask & src_vertex_lane_mask) != 0x0) ? src_vertex_lane_idx : get_ls1b(acc, control_mask));
            ...
            //
            if ( lane_idx == local_rep_lane_idx ) {
            ...
            const auto local_neigh_mask = inner_neigh_masks[ dst_vertex_idx ];
          ...
          for( auto idx : uniform_group_elements(acc, group, round_up_by(nVertices, w_extent)) ) { 
            ...
            //  
            const int neigh_mask = inner_neigh_masks[ dst_vertex_idx ];
            //
            const int neigh_num = alpaka::popcount(acc, neigh_mask);
            // Do warp-local accumulation:
            if ( neigh_num == 0 ) continue;

            const int src_vertex_idx = base_neighbor[dst_vertex_idx];
            //
          ...
          for( auto idx : uniform_group_elements(acc, group, round_up_by(nVertices, w_extent)) ) { 
            ...
            warp::syncWarpThreads_mask(acc, active_lanes_mask);
            //
            const unsigned warp_neigh_begin = warp::shfl_down_mask(acc, active_lanes_mask, begin, 1, w_extent);
            const unsigned end  = lane_idx == (warp_work_extent-1) ? nlist_offsets[idx.local+1] : warp_neigh_begin;
            //
          ...
          for( auto idx : uniform_group_elements(acc, group, round_up_by(nVertices, w_extent)) ) { 
            ...
            const auto nnz_mask = warp::ballot_mask(acc, active_lanes_mask, nnz > 0);
