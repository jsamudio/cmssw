...
namespace {
  ...
  //! 
  static thread_local CaloCellGeometry::CornersMgr s_cornersMgr( 65536, CaloCellGeometry::k_cornerSize); //k_cornerSize = 8;
...
static inline reco::PFRecHit makePFRecHit(PFLayer::Layer layer,
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
                              ...
                              const reco::PFRecHitDeviceCollection &pfRecHit ) const 
	{
	  ...
	  alpaka::exec<Acc1D>(queue, workDiv, ShowerShapeKernel{}, mdpfClusteringVars.view(), pfClusters.view(), pfRecHitFracs.view(), pfRecHit.view() );
...
using namespace ALPAKA_ACCELERATOR_NAMESPACE;
  ...
  std::pair<int, int> create(::reco::PFClusterCollection &clusters,
    ...
    for (int i = 0; i < nClusters; ++i) {
      ...
      for (int j = 0; j < nRH; ++j) { w[j] = wRaw_distr(rng); sumW += static_cast<double>(w[j]); }
      for (int j = 0; j < nRH; ++j) { w[j] /= sumW; }
      ...
      for (int j = 0; j < nRH; ++j) {
        ...
        if (current_energy > best_energy) { best_energy = current_energy; seed_idx = hIdx; }
      ...
      if (seed_idx >= 0) cluster.setSeed(hits[seed_idx].detId());
      else               cluster.setSeed(DetId(0));
    ...
    for (int i = 0; i < nHits; i++) {
      if (uni_distr(rng) > shareProbability) continue;
      ...
      float ownerFrac = 0.f; int ownerEntry = -1;

      for (int j = nFracs-1; j >= 0; --j) {
        if (rhfracs[j].pfrhIdx == i && rhfracs[j].pfcIdx == owner_idx) { ownerFrac = rhfracs[j].frac; ownerEntry = j; break; }
      }

      if (ownerEntry < 0) continue;

      const float leftover = std::max(0.f, 1.f - ownerFrac);
      if (leftover <= 0.f) continue;
      ...
      // keep total ≤ 1 by reducing the owner's entry accordingly (optional)
      rhfracs[ownerEntry].frac -= share_frac;
      if (rhfracs[ownerEntry].frac < 0.f) rhfracs[ownerEntry].frac = 0.f;
      ++nFracs;
    }

    if(verbose) printf("Generated cluster/rechit collections with %d hits and %d rechit fractions.\n", nHits, nFracs);  
  ...
  void checkShowerShapes( const ::reco::PFClusterCollection& clusters,
    ...
    for (int i = 0; i < nClusters; ++i) {
      ...
      etaRMS2[i] = std::max(etaSum / cluster.energy(), 0.00000001);
      etaRMS2[i] *= etaRMS2[i];
      phiRMS2[i] = std::max(phiSum / cluster.energy(), 0.00000001);
      phiRMS2[i] *= phiRMS2[i];
    }
