...
class PFJetAnalyzerHLTDQM : public DQMEDAnalyzer {
public:
  PFJetAnalyzerHLTDQM(const edm::ParameterSet&);
...
private:
  ...
  edm::EDGetTokenT<edm::View<reco::PFJet>> recoJetsToken;
  edm::EDGetTokenT<edm::View<reco::GenJet>> genJetsToken;
  edm::EDGetTokenT<reco::CandViewMatchMap> srcRefToJetMap;

  void fillJetResponse(edm::View<reco::PFJet>& recoJetCollection, edm::View<reco::GenJet>& genJetCollection);
...
void PFJetAnalyzerHLTDQM::prepareJetResponsePlots(const std::vector<edm::ParameterSet>& response_plots) {
...
void PFJetAnalyzerHLTDQM::prepareGenJetPlots(const std::vector<edm::ParameterSet>& genjet_plots_pset) {
...
void PFJetAnalyzerHLTDQM::prepareGenJetMatchedPlots(const std::vector<edm::ParameterSet>& genjet_plots_pset) {
  for (auto& pset : genjet_plots_pset) {
...
void PFJetAnalyzerHLTDQM::prepareGenJetUnmatchedPlots(const std::vector<edm::ParameterSet>& genjet_plots_pset) {
  for (auto& pset : genjet_plots_pset) {
...
void PFJetAnalyzerHLTDQM::prepareRecoJetPlots(const std::vector<edm::ParameterSet>& recojet_plots_pset) {
...
void PFJetAnalyzerHLTDQM::prepareRecoJetMatchedPlots(const std::vector<edm::ParameterSet>& recojet_plots_pset) {
  for (auto& pset : recojet_plots_pset) {
    const auto name = pset.getParameter<std::string>("name") + "_matched";
    const auto title = "Matched " + pset.getParameter<std::string>("title");
...
void PFJetAnalyzerHLTDQM::prepareRecoJetUnmatchedPlots(const std::vector<edm::ParameterSet>& recojet_plots_pset) {
  for (auto& pset : recojet_plots_pset) {
    const auto name = pset.getParameter<std::string>("name") + "_unmatched";
    const auto title = "Unmatched " + pset.getParameter<std::string>("title");
...
PFJetAnalyzerHLTDQM::PFJetAnalyzerHLTDQM(const edm::ParameterSet& iConfig) {
  ...
  recoJetsToken = consumes<edm::View<reco::PFJet>>(recoJetsLabel);
...
void PFJetAnalyzerHLTDQM::bookHistograms(DQMStore::IBooker& booker, edm::Run const&, edm::EventSetup const&) {
...
void PFJetAnalyzerHLTDQM::analyze(const edm::Event& iEvent, const edm::EventSetup&) {
  edm::Handle<edm::View<reco::PFJet>> recoJetCollectionHandle;
  ...
  if (isMC) {
    edm::Handle<edm::View<reco::GenJet>> genJetCollectionHandle;
...
#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(PFJetAnalyzerHLTDQM);
